...
export function createAgentHubView(parent) {
...
function renderConnectedAgents(parent, agents) {
    const lines = [];
...
function renderActivityFeed(parent, log, agents) {
    const lines = [];
...
function renderAgentPerformance(parent, agents, log) {
    const top = parent.children.length > 0 ? parent.children[0].height + 1 : 20;
    if (agents.length === 0)
        return;
    const now = new Date();
