import { readTracker, writeTracker, findTask, touchAgent, autoUnblockDependents, TrackerState, Subtask, Milestone, Agent } from './tracker.js';
import { buildTaskContext, buildTaskSummary, buildProjectStatus, buildMilestoneOverview } from './context.js';

type ToolHandler = (args: Record<string, unknown>) => string | { text: string; isError: boolean };

function makeTool(name: string, description: string, inputSchema: object, handler: ToolHandler) {
  return {
    name,
    description,
    inputSchema,
...
export const getTaskContextTool = makeTool(
  'get_task_context',
  ...
  (args) => {
    ...
    const found = findTask(state, args.task_id as string);
    if (!found) return { text: `Task '${args.task_id}' not found in any milestone`, isError: true };
    return buildTaskContext(state, found.subtask, found.milestone);
  }
);

export const getTaskSummaryTool = makeTool(
  'get_task_summary',
  'Get slim summary for a task',
  {
    type: 'object',
    properties: { task_id: { type: 'string', description: 'The subtask ID' } },
    required: ['task_id']
...
export const getProjectStatusTool = makeTool(
  'get_project_status',
  'Get overall project status',
  { type: 'object', properties: {} },
  () => buildProjectStatus(readTracker())
);

export const getMilestoneOverviewTool = makeTool(
  'get_milestone_overview',
  'Get overview for a milestone',
  {
    type: 'object',
  ...
  (args) => {
    ...
    if (!milestone) return { text: `Milestone '${args.milestone_id}' not found`, isError: true };
    return buildMilestoneOverview(milestone, state);
  }
);

export const listTasksTool = makeTool(
  'list_tasks',
...
export const getTaskHistoryTool = makeTool(
  'get_task_history',
...
export const listAgentsTool = makeTool(
  'list_agents',
...
export const getActivityFeedTool = makeTool(
  'get_activity_feed',
  ...
  (args) => {
    ...
    logs = logs.slice(0, (args.limit as number) || 30);
...
export const startTaskTool = makeTool(
  ...
  (args) => {
    const state = readTracker();
    const found = findTask(state, args.task_id as string);
    if (!found) return { text: `Task '${args.task_id}' not found`, isError: true };
    
    found.subtask.status = 'in_progress';
    found.subtask.last_run_id = 'run_' + Date.now();
    if (!found.subtask.assignee) {
      found.subtask.assignee = args.agent_id as string;
    }
    ...
    touchAgent(state, args.agent_id as string);
    writeTracker(state);
...
export const completeTaskTool = makeTool(
  'complete_task',
  'Mark task as complete (for review)',
  {
    type: 'object',
    properties: {
      task_id: { type: 'string', description: 'The subtask ID' },
...
export const approveTaskTool = makeTool(
  'approve_task',
  'Approve a completed task (operator only)',
  {
    type: 'object',
    properties: {
      task_id: { type: 'string', description: 'The subtask ID' },
  ...
  (args) => {
    ...
    found.subtask.completed_by = args.operator_id as string;
    ...
    state.agent_log.push({
      id: 'log_' + Date.now() + '_' + Math.random().toString(36).slice(2, 7),
      agent_id: args.operator_id as string,
      action: 'task_approved',
      target_type: 'subtask',
      target_id: found.subtask.id,
      description: (args.feedback as string) || 'Approved',
      timestamp: new Date().toISOString(),
      tags: ['approved']
    });
    
    touchAgent(state, args.operator_id as string);
    writeTracker(state);
...
export const rejectTaskTool = makeTool(
  'reject_task',
  'Reject a task for revision',
  {
    type: 'object',
    properties: {
      task_id: { type: 'string', description: 'The subtask ID' },
...
export const resetTaskTool = makeTool(
  'reset_task',
...
export const blockTaskTool = makeTool(
  'block_task',
  'Block a task',
  {
    type: 'object',
  ...
  (args) => {
    ...
    found.subtask.blocked_reason = args.reason as string;
    found.subtask.blocked_by = args.agent_id as string;
...
export const unblockTaskTool = makeTool(
  'unblock_task',
  'Unblock a task',
  {
    type: 'object',
  ...
  (args) => {
    ...
    state.agent_log.push({
      ...
      target_type: 'subtask',
      target_id: found.subtask.id,
      description: (args.resolution as string) || `Resolved: ${prevReason}`,
      timestamp: new Date().toISOString(),
      tags: ['unblocked']
    });
...
export const updateTaskTool = makeTool(
  'update_task',
  'Update task fields',
  {
    type: 'object',
  ...
  (args) => {
    ...
    if (args.priority && found.subtask.priority !== args.priority) {
      found.subtask.priority = args.priority as string;
    ...
    if (args.execution_mode && found.subtask.execution_mode !== args.execution_mode) {
      found.subtask.execution_mode = args.execution_mode as 'human' | 'agent' | 'pair';
...
export const logActionTool = makeTool(
  'log_action',
...
export const enrichTaskTool = makeTool(
  'enrich_task',
  'Enrich task with additional details',
  {
    type: 'object',
    properties: {
      task_id: { type: 'string' },
  ...
  (args) => {
    ...
    if (args.prompt !== undefined) {
      found.subtask.prompt = args.prompt as string || null;
      changes.push('prompt');
    }
    if (args.builder_prompt !== undefined) {
      found.subtask.builder_prompt = args.builder_prompt as string || null;
      changes.push('builder_prompt');
    }
    if (args.acceptance_criteria !== undefined) {
      found.subtask.acceptance_criteria = args.acceptance_criteria as string[] || [];
      changes.push('acceptance_criteria');
    }
    if (args.constraints !== undefined) {
      found.subtask.constraints = args.constraints as string[] || [];
      changes.push('constraints');
    }
    if (args.context_files !== undefined) {
      found.subtask.context_files = args.context_files as string[] || [];
      changes.push('context_files');
    }
    if (args.reference_docs !== undefined) {
      found.subtask.reference_docs = args.reference_docs as string[] || [];
...
export const addMilestoneNoteTool = makeTool(
  'add_milestone_note',
  ...
  (args) => {
    ...
    milestone.notes.push(args.note as string);
...
export const setMilestoneDatesTool = makeTool(
  'set_milestone_dates',
  'Set actual dates for a milestone',
  {
    type: 'object',
    properties: {
      milestone_id: { type: 'string' },
  ...
  (args) => {
    ...
    if (args.actual_start) {
      milestone.actual_start = args.actual_start as string;
    ...
    if (args.actual_end) {
      milestone.actual_end = args.actual_end as string;
    }
...
export const updateDriftTool = makeTool(
  'update_drift',
  'Update milestone drift days',
  {
    type: 'object',
    properties: {
      milestone_id: { type: 'string' },
  ...
  (args) => {
    ...
    milestone.drift_days = args.drift_days as number;
...
export const createMilestoneTool = makeTool(
  'create_milestone',
  ...
  (args) => {
    ...
    const milestone: Milestone = {
      ...
      phase: args.phase as string || args.id as string,
      week: state.milestones.length + 1,
      planned_start: args.planned_start as string || null,
      planned_end: args.planned_end as string || null,
      actual_start: null,
      actual_end: null,
      drift_days: 0,
...
export const addMilestoneTaskTool = makeTool(
  'add_milestone_task',
  ...
  (args) => {
    ...
    const task: Subtask = {
      ...
      agent_target: null,
      execution_mode: args.execution_mode as 'human' | 'agent' | 'pair' || 'agent',
      depends_on: args.depends_on as string[] || [],
      last_run_id: null,
      builder_prompt: null
    };
...
export const registerAgentTool = makeTool(
  'register_agent',
  'Register an agent',
  {
    type: 'object',
  ...
  (args) => {
    ...
    if (existing) {
      ...
      existing.type = args.type as 'orchestrator' | 'sub-agent' | 'human' | 'external';
      existing.permissions = args.permissions as string[];
      if (args.color) existing.color = args.color as string;
      if (args.parent_id) existing.parent_id = args.parent_id as string;
      ...
      const agent: Agent = {
        id: args.agent_id as string,
        name: args.name as string,
        type: args.type as 'orchestrator' | 'sub-agent' | 'human' | 'external',
        color: args.color as string || '#9B9BAA',
        status: 'active',
        permissions: args.permissions as string[],
        last_action_at: new Date().toISOString(),
        session_action_count: 0
      };
      if (args.parent_id) agent.parent_id = args.parent_id as string;
