#!/usr/bin/env node
import { readTracker, writeTracker, findTask, touchAgent } from './tracker.js';
    ...
    try {
        ...
        switch (command) {
            ...
            case 'get-task-context': {
                const taskId = args[1];
                if (!taskId)
                    error('Usage: get-task-context <task_id>');
            ...
            case 'get-task-summary': {
                const taskId = args[1];
                if (!taskId)
                    error('Usage: get-task-summary <task_id>');
                const found = findTask(state, taskId);
                if (!found)
                    error(`Task '${taskId}' not found`);
            ...
            case 'get-milestone-overview': {
                ...
                if (!milestoneId)
                    error('Usage: get-milestone-overview <milestone_id>');
            ...
            case 'list-tasks': {
                ...
                if (milestoneFilter)
                    tasks = tasks.filter(t => t._milestone.id === milestoneFilter);
                if (statusFilter)
                    tasks = tasks.filter(t => t.status === statusFilter);
                if (domainFilter)
                    tasks = tasks.filter(t => t._milestone.domain === domainFilter);
                result = '# Tasks\n';
            ...
            case 'get-activity-feed': {
                ...
                if (agentFilter)
                    logs = logs.filter(l => l.agent_id === agentFilter);
            ...
            case 'start-task': {
                const taskId = args[1];
                if (!taskId)
                    error('Usage: start-task <task_id>');
                const found = findTask(state, taskId);
                if (!found)
                    error(`Task '${taskId}' not found`);
            ...
            case 'complete-task': {
                ...
                if (!taskId || !summary)
                    error('Usage: complete-task <task_id> "<summary>"');
                const found = findTask(state, taskId);
                if (!found)
                    error(`Task '${taskId}' not found`);
                const agentId = getArg('--agent', 'orchestrator');
            ...
            case 'approve-task': {
                const taskId = args[1];
                if (!taskId)
                    error('Usage: approve-task <task_id>');
                const found = findTask(state, taskId);
                if (!found)
                    error(`Task '${taskId}' not found`);
                const { subtask, milestone } = found;
                if (subtask.status !== 'review')
                    error(`Task '${taskId}' is in status '${subtask.status}', expected 'review'`);
            ...
            case 'reject-task': {
                ...
                if (!taskId || !feedback)
                    error('Usage: reject-task <task_id> "<feedback>"');
                const found = findTask(state, taskId);
                if (!found)
                    error(`Task '${taskId}' not found`);
                const { subtask } = found;
            ...
            case 'reset-task': {
                const taskId = args[1];
                if (!taskId)
                    error('Usage: reset-task <task_id>');
                const found = findTask(state, taskId);
                if (!found)
                    error(`Task '${taskId}' not found`);
                const { subtask } = found;
            ...
            case 'block-task': {
                ...
                const reason = args[2];
                if (!taskId || !reason)
                    error('Usage: block-task <task_id> "<reason>"');
                const found = findTask(state, taskId);
                if (!found)
                    error(`Task '${taskId}' not found`);
                const agentId = getArg('--agent', 'orchestrator');
            ...
            case 'unblock-task': {
                const taskId = args[1];
                if (!taskId)
                    error('Usage: unblock-task <task_id>');
                const found = findTask(state, taskId);
                if (!found)
                    error(`Task '${taskId}' not found`);
                const { subtask } = found;
            ...
            case 'update-task': {
                const taskId = args[1];
                if (!taskId)
                    error('Usage: update-task <task_id> [...]');
                const found = findTask(state, taskId);
                if (!found)
                    error(`Task '${taskId}' not found`);
                const agentId = getArg('--agent', 'orchestrator');
                ...
                if (executionMode && subtask.execution_mode !== executionMode) {
                    subtask.execution_mode = executionMode;
            ...
            case 'log-action': {
                ...
                if (!targetId || !action || !description)
                    error('Usage: log-action <target_id> <action> "<description>"');
                const agentId = getArg('--agent', 'orchestrator');
            ...
            case 'enrich-task': {
                const taskId = args[1];
                if (!taskId)
                    error('Usage: enrich-task <task_id> [...]');
                const found = findTask(state, taskId);
                if (!found)
                    error(`Task '${taskId}' not found`);
                const agentId = getArg('--agent', 'orchestrator');
            ...
            case 'add-milestone-note': {
                ...
                const note = args[2];
                if (!milestoneId || !note)
                    error('Usage: add-milestone-note <milestone_id> "<note>"');
                const milestone = state.milestones.find(m => m.id === milestoneId);
                if (!milestone)
                    error(`Milestone '${milestoneId}' not found`);
            ...
            case 'set-milestone-dates': {
                const milestoneId = args[1];
                if (!milestoneId)
                    error('Usage: set-milestone-dates <milestone_id> [...]');
            ...
            case 'update-drift': {
                ...
                if (!milestoneId || isNaN(driftDays))
                    error('Usage: update-drift <milestone_id> <days>');
                let milestone = state.milestones.find(m => m.id === milestoneId);
                if (!milestone)
                    error(`Milestone '${milestoneId}' not found`);
                const ms = milestone;
            ...
            case 'create-milestone': {
                ...
                const title = args[2];
                if (!id || !title)
                    error('Usage: create-milestone <id> "<title>" [...]');
                if (state.milestones.find(m => m.id === id))
                    error(`Milestone '${id}' already exists`);
                const milestone = {
                    id,
                    title,
            ...
            case 'add-milestone-task': {
                const milestoneId = args[1];
                const label = args[2];
                if (!milestoneId || !label)
                    error('Usage: add-milestone-task <milestone_id> "<label>" [...]');
                let milestone = state.milestones.find(m => m.id === milestoneId);
                if (!milestone)
                    error(`Milestone '${milestoneId}' not found`);
                const ms = milestone;
                ...
                const task = {
                    ...
                    constraints: [],
                    agent_target: null,
                    execution_mode: getArg('--execution_mode', 'agent'),
                    depends_on: [],
                    last_run_id: null,
                    builder_prompt: null
                };
            ...
            case 'register-agent': {
                ...
                if (!agentId || !name || !type)
                    error('Usage: register-agent <id> <name> <type> --permissions read,write');
